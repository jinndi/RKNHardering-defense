...
- Рекомендация: Для прохождения проверок на утечки (CDN Pulling) **на Android-смартфонах лучше попрежднему использовать приложение `sing-box`**. При этом само устройство должно быть подключено к Wi-Fi сегменту роутера, где трафик идет напрямую только через провайдера без резерыных подключений.
