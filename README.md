...
3. Переходим в:  
   `Настройки → JSON подписка → Редактор`

   Добавляем следующий шаблон:

   > ⚠️ Укажите свои доверенные приложения в поле `inbounds.tun.include_package`, имена можно узнать через приложение `Package Names Viewer` из `Google Play`
